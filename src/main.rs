...
impl Context {
    ...
    fn fresh_deffered_id(&mut self) -> u32 {
        let ret = self.deffered_count;
        self.deffered_count += 1;
        ret
    }
}

fn abs_append_inv(target: StrLenLat, lhs: StrLenLat) -> StrLenLat {
    println!("Solved abstraction: {:?} {:?}", target.clone(), lhs.clone());
    target - lhs
}
...
fn gen_append(ctx: &mut Context, abs: &StrLenLat) -> Expr<AbsStrVal, StrLenLat> {
    let deffered_id = ctx.fresh_deffered_id();
...
fn concretize(ctx: &mut Context, size: u32) -> Vec<Expr<AbsStrVal, StrLenLat>> {
    ...
    match cached {
        ...
        None => {
            ...
            // let all_exprs: Vec<Expr<AbsStrVal, StrLenLat>> =
            //     appends.into_iter().chain(replaces.into_iter()).collect();

            let all_exprs = appends;
        ...
        Expr::Hole(abs, Some(e)) => match *e {
            Func::Append(Expr::ConcHole(size), Expr::DepHole) => {
                ...
                let rhs_abs_vec = lhs_vec
                    ...
                    .into_iter()
                    .filter_map(|x| {
                        println!("log {:?}", StrLenInterp::eval(&x, &env));
                        match StrLenInterp::eval(&x, &env).ok() {
...
fn main() {
    ...
    env.insert("arg0".to_string(), AbsStrVal::from(5));

    let target = LinearExpr::from(LinearExpr::from(11));
